'''
Easy Apply modal form-filling: resume upload and answering select/radio/text/textarea/checkbox questions.
'''
...
from bot.shared import randomly_answered_questions, add_unique
...
# Derived constants (computed once from config; never mutated afterward)
...
def answer_questions(modal: WebElement, questions_list: list, work_location: str, current_city: str, ai_client, job_description: str | None = None ) -> list:
    ...
    for Question in all_questions:
            ...
            if overwrite_previous_answers or selected_option == "Select an option":
                ...
                try:
                    ...
                    if not foundOption:
                        ...
                        add_unique(randomly_answered_questions, (f'{label_org} [ {options} ]',"select"))
            add_unique(questions_list, (f'{label_org} [ {options} ]', answer, "select", prev_answer))
            continue
        ...
        if radio:
            ...
            if overwrite_previous_answers or prev_answer is None:
                ...
                if foundOption:
                    ...
                    if not foundOption: add_unique(randomly_answered_questions, (f'{label_org} ]',"radio"))
            else: answer = prev_answer
            add_unique(questions_list, (label_org+" ]", answer, "radio", prev_answer))
            continue
        ...
        if text:
                    ...
                    if use_AI and ai_client:
                        try:
                            if ai_provider.lower() == "openai":
                                ...
                                add_unique(randomly_answered_questions, (label_org, "text"))
                                answer = years_of_experience
            ...
            add_unique(questions_list, (label, text.get_attribute("value"), "text", prev_answer))
            continue
        ...
        if text_area:
                    ...
                    if use_AI and ai_client:
                        try:
                            if ai_provider.lower() == "openai":
                                ...
                                add_unique(randomly_answered_questions, (label_org, "textarea"))
                                answer = ""
                            if answer and isinstance(answer, str) and len(answer) > 0:
                                print_lg(f'AI Answered received for question "{label_org}" \nhere is answer: "{answer}"')
                            else:
            ...
            if do_actions:
                    ...
                    actions.send_keys(Keys.ARROW_DOWN)
                    actions.send_keys(Keys.ENTER).perform()
            add_unique(questions_list, (label, text_area.get_attribute("value"), "textarea", prev_answer))
            continue
        ...
        if checkbox:
            ...
            if not prev_answer:
                try:
                    ...
                    pass
            add_unique(questions_list, (f'{label} ([X] {answer})', checked, "checkbox", prev_answer))
            continue
